...
def select_files():
    ...
    for path, name in [(expected_noise_path, "Noise"), (expected_shap_path, "SHAP")]:
        if not os.path.exists(path):
            logger.error(
                f"Could not find the corresponding {name} file: '{os.path.basename(path)}'"
            )
            return None, None, None
...
def calculate_eta_raw(
    ...
    """
    logger.info(
        f"Starting calculation of η_raw with segment_ms={segment_ms} and percentile={percentile}..."
    )
    ...
    if shap_matrix.shape[0] != clean_audio.shape[0]:
        if shap_matrix.shape[1] == clean_audio.shape[0]:
            ...
            raise ValueError(
                f"SHAP matrix shape {shap_matrix.shape} is incompatible with audio length {len(clean_audio)}."
            )
    ...
    logger.info(
        f"Processing audio into {num_segments} segments of {segment_length_samples} samples each."
    )
    ...
    logger.info(
        f"Generated Ideal Time-domain Mask (ITM). Found {np.sum(itm)} speech-dominated segments."
    )
    ...
    logger.info(
        f"Calculated relevance threshold τ = {tau:.4f} ({percentile}th percentile)."
    )
    ...
    if denominator == 0:
        logger.warning(
            "No segments were identified as highly relevant (denominator is 0). Returning η_raw = 0."
        )
        return 0.0
    intersection = is_highly_relevant * itm
    numerator = np.sum(intersection)
    logger.info(
        f"Found {numerator} segments in the intersection (highly relevant AND speech-dominated)."
    )
...
if __name__ == "__main__":
    ...
    if shap_values_matrix.ndim > 2:
        logger.warning(
            f"Original SHAP matrix has {shap_values_matrix.ndim} dimensions (shape: {shap_values_matrix.shape}). Squeezing to 2D."
        )
        shap_values_matrix = np.squeeze(shap_values_matrix)
        if shap_values_matrix.ndim > 2:
            logger.error(
                "Squeezing failed to reduce SHAP matrix to 2 dimensions. Cannot proceed."
            )
            sys.exit(1)
