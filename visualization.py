...
class InteractiveShapVisualizer:
    # --- CHANGE: Added `list_of_masks` to __init__ ---
    def __init__(self, audio_data, sr, transcription, char_shap_audios_list, display_tokens, list_of_masks):
        ...
        # --- CHANGE: Adjust subplot to make room for buttons at the bottom ---
        ...
        self._setup_plots()
        # --- NEW: Setup the buttons ---
        if self.display_buttons : self._setup_buttons()
        self.fig.canvas.mpl_connect('button_press_event', self._on_click)

    # --- NEW: Method to create and setup buttons ---
    ...
    def _on_click(self, event):
        ...
        for i, text_obj in enumerate(self.text_objects):
            ...
            if contains:
                ...
                # --- NEW: Activate buttons on first selection ---
    ...
    # (The rest of the class methods are unchanged and omitted for brevity)
    def _plot_token_timeline(self):
        ...
        for time, token in zip(time_coords, self.display_tokens):
            ax.text(time, 0.5, token, ha='center', va='center', fontsize=9, fontfamily='monospace')
        ax.set_ylabel('Tokens', rotation=0, ha='right', va='center', fontsize=10, labelpad=10)
    ...
    def _setup_plots(self):
        ...
        librosa.display.specshow(S_db_original, sr=self.sr, x_axis='time', y_axis='mel', ax=self.ax_main_spec)
        ...
        self.ax_text.set_title(f'Clickable Transcription: "{self.full_transcription}"', style='italic')
        ...
        self.ax_shap_spec.set_title('Click on a letter to see its SHAP-weighted Spectrogram', fontsize=14)
    ...
    def _update_shap_spectrogram(self, char_index):
        if char_index >= len(self.char_shap_audios_list):
            logger.warning(f"Character index {char_index} out of bounds for SHAP audio list.")
            return
        ...
        librosa.display.specshow(S_db_shap, sr=self.sr, x_axis='time', y_axis='mel', ax=self.ax_shap_spec)
        self.ax_shap_spec.set_title(f"SHAP-weighted Spectrogram for letter: '{char}' at position {char_index+1}", fontsize=14)
        self.fig.canvas.draw_idle()
...
def load_and_process_data(audio_path, shap_path):
    ...
    # (Sections 1, 2, 2a, and 3 are unchanged...)
    # 1. Load Model
    ...
    if len(char_timestep_indices) != len(clean_transcription):
        logger.error(f"Mismatch between decoded characters ({len(clean_transcription)}) and found token groups ({len(char_timestep_indices)}).")
        return audio, sr, transcription, [], [], [] # Added one more empty list
    ...
    try:
        ...
        if shap_values.shape != expected_shape:
            raise ValueError(f"Unexpected SHAP shape. Got {shap_values.shape}, expected {expected_shape}.")
    except Exception as e:
    ...
    # --- START OF CHANGE ---
    list_of_masks = [] # Create a list to store the masks
    # --- END OF CHANGE ---
    for i, char_timestep_index in enumerate(char_timestep_indices):
        ...
        # --- START OF CHANGE ---
        list_of_masks.append(normalized_shap) # Store the mask
        # --- END OF CHANGE ---
    
    logger.info(f"Processed SHAP values for {len(list_of_amplified_audios)} characters.")
    # --- CHANGE: Return the new list_of_masks ---
    return audio, sr, transcription, list_of_amplified_audios, display_tokens, list_of_masks
...
def select_file_from_data_folder():
    ...
    if not os.path.exists(expected_shap_path):
        ...
        logger.error("Please ensure for every 'audio_X.npy' there is a 'shap_values_X.npy'.")
        return None, None
...
def main():
    ...
    # --- CHANGE: Receive the new list_of_masks ---
    audio, sr, transcription, char_audios_list, display_tokens, list_of_masks = load_and_process_data(audio_path, shap_path)
    ...
    if not char_audios_list or not list_of_masks:
        logger.error("SHAP audio or mask data list is empty. Cannot start visualization.")
        return

    # --- CHANGE: Pass the new list to the visualizer ---
    visualizer = InteractiveShapVisualizer(audio, sr, transcription, char_audios_list, display_tokens, list_of_masks)
    plt.show()
