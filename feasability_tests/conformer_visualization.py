...
class ModelWrapper(torch.nn.Module):
    ...
    def forward(self, x):
        (log_probs, encoded_len, greedy_predictions) = self.model.forward(
            processed_signal=x,
            processed_signal_length=torch.tensor([x.shape[2]]).to(device=device),
        )
...
plt.imshow(
    shap_values[0][0].cpu().numpy(), aspect="auto", origin="lower", cmap="viridis"
)
